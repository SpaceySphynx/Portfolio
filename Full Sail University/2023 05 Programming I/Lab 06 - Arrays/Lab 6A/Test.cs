...
namespace Tester
{
    class Test
    {
        ...
        public static void Run(int labNumber)
        {
            ...
            labScore += testScore;
            UI.DisplayPrograssBar(testScore, 10, 20, Submission.GAME_MODE);
            ...
            for (int ndx = 0; ndx < seeds.Length; ndx++)
            {
                ...
                //     source.ToString(),
                //     expected.ToString(),
                //     submitted != null ? submitted.ToString() : "null",
                //     10,
                //     10
                // );

                Console.Write($"Pass {ndx + 1} of 5: [");
                if (success)
                {
                ...
                Console.Write("Input:\n");
                Print2DArray(source);
                Console.CursorTop -= (source.GetLength(0) + 1);
                Console.CursorLeft = 15;

                Console.Write("Expected:\n");
                Print2DArray(expected, 15);
                Console.CursorTop -= (expected.GetLength(0) + 1);


                Console.CursorLeft = 30;
                Console.Write("Submitted:\n");
                Print2DArray(submitted, 30);

                Console.CursorLeft = 0;
                Console.CursorTop += (submitted == null) ? expected.GetLength(1) : 0;
                Console.Write("\n\n--\n");
            ...
            for (int ndx = 0; ndx < seed2s.Length; ndx++)
            {
                ...
                Console.Write("Expected:\n");
                Print2DArray(expected, 12);
                Console.CursorTop -= (expected.GetLength(0) + 1);


                Console.CursorLeft = 27;
                Console.Write("Submitted:\n");
                Print2DArray(submitted, 27);

                Console.CursorLeft = 0;
                Console.CursorTop += (submitted == null) ? expected.GetLength(0) : 0;
                Console.Write("\n\n--\n");
        ...
        public static void Print2DArray(int[,] array, int xPosition = 0)
        {
            // Console.Write('\n');
            Console.ForegroundColor = ConsoleColor.DarkYellow;
        ...
        public static void Normalize(double[] source)
        {
            ...
            {
                source[ndx] = Math.Round(source[ndx] / largest, 3);
            }
        }
