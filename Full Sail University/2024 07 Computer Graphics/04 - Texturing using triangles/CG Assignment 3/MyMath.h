...
inline unsigned int int_Lerp(int a, int b, float ratio)
{
    return static_cast<unsigned int>(a + (b - a) * ratio);
}

// Linear interpolation between ints a and b but into a float using the ratio (between 0 and 1).
inline float float_Lerp(int a, int b, float ratio)
{
    return a + (b - a) * ratio;
}

// Converts 2D x,y coordinates to a 1D array for backbuffer.
inline unsigned int Convert2Dto1D(unsigned int x, unsigned int y, unsigned int width)
{
    return y * width + x;
}

// Convert coordinates (NDC) to screen space.
inline int ConvertToScreenX(float ndcX, unsigned int screenWidth)
{
    return static_cast<int>((ndcX + 1.0f) * 0.5f * screenWidth);
}

// Convert coordinates (NDC) to screen space for left-handed system.
inline int ConvertToScreenY(float ndcY, unsigned int screenHeight)
{
    return static_cast<int>((ndcY + 1.0f) * 0.5f * screenHeight);
}

// Computes the dot product of two 4D vectors.
inline float DotProduct(const Float4& v1, const Float4& v2)
{
    return (v1.x * v2.x) + (v1.y * v2.y) + (v1.z * v2.z);
}

// Implicit line equation to check which side of a line a point lies on.
inline float ImplicitLineEquation(float X1, float X2, float Y1, float Y2, float fTestX, float fTestY)
{
    return (Y2 - Y1) * (fTestX - X1) - (X2 - X1) * (fTestY - Y1);
}
...
// Creates a rotation matrix around the each axis.
void Rotate(Matrix4x4& matrix, const char axis, float degrees)
{
...
// Perspective projection maatrix.
void ProjectionMatrix(Matrix4x4& matrix, float fov, float aspectRatio, float nearPlane, float farPlane)
{
    ...
    matrix.AxisX.x = xScale;
    matrix.AxisY.y = yScale;
    matrix.AxisZ.z = farPlane / (farPlane - nearPlane); // 1.01 expected.
    matrix.AxisZ.w = 1.0f;
    matrix.AxisW.z = -(farPlane * nearPlane) / (farPlane - nearPlane); // -0.1 expected.
    matrix.AxisW.w = 0.0f;
}
...
// Finds the barycentric coordinates (alpha, beta, gamma) for a point (x, y) within a triangle (A, B, C).
void FindBarycentricCoords(float Px, float Py, float Ax, float Ay, float Bx, float By, float Cx, float Cy, float& alpha, float& beta, float& gamma)
{
    float denominator = ImplicitLineEquation(Ax, Bx, Ay, By, Cx, Cy);

    // Avoid division by zero or very small values.
    if (fabs(denominator) > 1e-6f)
    {
        alpha = ImplicitLineEquation(Bx, Cx, By, Cy, Px, Py) / denominator; // Weight for vertex A.
        beta = ImplicitLineEquation(Cx, Ax, Cy, Ay, Px, Py) / denominator;  // Weight for vertex B.
        gamma = ImplicitLineEquation(Ax, Bx, Ay, By, Px, Py) / denominator; // Weight for vertex C.
    }
    else
    {
        alpha = beta = gamma = -1.0f; // Out of bounds.
    }
}
